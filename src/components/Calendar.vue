<template>
  <div id="calendar">
    ...
    <div class="content">
      ...
      <ul class="month">
        <li v-for="(item,index) in days" :key="index" @click="handleDayChoose(item,index)" :class="[item.className,dayClasses(item,index)]">
          <span>{{item.day}}</span>
        </li>
      </ul>
    </div>
  ...
  methods: {
    dayClasses(item, index) {
      ...
      return {
        ...
        "disabled-day": this.disabledFuture && item.isFutureDay,
        "other-month-day": isOtherMonth,
        "other-month-day--hide": this.hideOtherMonthDays && isOtherMonth
      };
    },
    /**
    ...
    initMonth() {
      ...
      let prevMonthDays = this.getTotalDays(
        year,
        month === 1 ? (month = 12) : month - 1
      );
      let nextMonthDays = this.getTotalDays(
        year,
